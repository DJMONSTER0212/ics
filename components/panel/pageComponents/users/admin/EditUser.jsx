...
const EditUser = ({ editUserDrawer, setEditUserDrawer, editMessage, setEditMessage, settings, reFetch, session, editUserId, setGlobalMessage }) => {
    ...
    const editFormSubmit = async (data) => {
        ...
        const response = await fetch(`/api/panel/users/admin/${data._id}`, {
            method: "POST",
            body: formData
        })
        const responseData = await response.json()
    ...
    const editFetch = useCallback(async (id) => {
        ...
        if (responseData.data) {
            ...
            if (responseData.data.dob) {
                responseData.data.dob = responseData.data.dob.slice(0, 10);
            }
            if (responseData.data.anniversary) {
                responseData.data.anniversary = responseData.data.anniversary.slice(0, 10);
            }

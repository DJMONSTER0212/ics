    ...
    form.parse(req, async (error, fields, files) => {
        ...
        try {
            ...
            if (req.method == 'POST') {
                ...
                // Adding location
                try {
                    const newLocation = new locationsModel({ ...updateField })
                    await newLocation.save();
                    return res.status(200).json({ success: 'Location has been added successfully' })
                } catch (error) {
                    return res.status(500).json({ error: `Adding location failed. Please try again. ${process.env.NODE_ENV == 'development' && `Error : ${error}`}` })
                }
                ...
                try {
                    ...
                    const data = await locationsModel.aggregate([
                        { $match: search ? SearchMatchQuery : matchQuery },
                        { $sort: sortQuery },
                        { $skip: Number(pageIndex) * Number(maxResult) },
                        { $limit: Number(maxResult) },
                    ]);
                    const totalRow = await locationsModel.countDocuments(search ? SearchMatchQuery : matchQuery);

    ...
    form.parse(req, async (error, fields, files) => {
        ...
        try {
            ...
            if (req.method == 'POST') {
                ...
                // Updating villa seo info >>>>>>>>>>>>>>
                try {
                    await villasModel.updateOne({ _id: fields._id }, { $set: updateFields });
                    return res.status(200).json({ success: 'SEO information has been updated successfully' })
                } catch (error) {
                    return res.status(500).json({ error: `Updating seo information failed. Please try again. ${process.env.NODE_ENV == 'development' && `Error : ${error}`}` })
                }
                ...
                try {
                    ...
                    return res.status(500).json({ error: `Fetching villa seo info failed. ${process.env.NODE_ENV == 'development' && `Error : ${error}`}` });
                }
            } else {

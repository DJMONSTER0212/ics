            ...
            if (req.method == 'POST') {
                ...
                if (fields.rangeType == 'date') {
                    if (new Date(fields.endDate) >= new Date(fields.startDate)) {
                        ...
                        const currentSeasonalPricings = await seasonalPricingsModel.find({ villaId: req.query.id, rangeType: 'date' });
                    ...
                    const currentSeasonalPricings = await seasonalPricingsModel.find({ villaId: req.query.id, rangeType: 'day', day: fields.day });
                ...
                // Delete empty fields >>>>>>>>>>>>>>
                function deleteEmptyFields(obj, fields) {
                ...
                deleteEmptyFields(updateFields, ['discountedPrice', 'extraGuestPrice', 'childPrice', 'petPrice']);
                // Updating data >>>>>>>>>>>>>>
                try {
                    const newSeasonalPricing = new seasonalPricingsModel(updateFields)
                    await newSeasonalPricing.save();
                    return res.status(200).json({ success: 'Seasonal pricing has been created successfully' })
                } catch (error) {
                    return res.status(500).json({ error: `Creating seasonal pricing failed. Please try again. ${process.env.NODE_ENV == 'development' && `Error : ${error}`}` })
                }
                ...
                try {
                    ...
                    return res.status(500).json({ error: `Fetching seasonal priicing failed. ${process.env.NODE_ENV == 'development' && `Error : ${error}`}` });
                }
            } else {

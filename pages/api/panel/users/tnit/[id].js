            ...
            if (req.method == 'POST') {
                ...
                if ((!(user.role == 'vendor' || user.role == 'admin') && (updateFields.role == 'vendor' || updateFields.role == 'admin'))) {
                    ...
                    if (!settings.tnit.multiVendorAllowed) {
                        return res.status(500).json({ error: `You are not allowed to create admin or vendor on single vendor model. Please enable multi vendor model in settings.` })
                    }
                    if (settings.tnit.limitMaxVendor == true) {
                        ...
                        if (totalVendor >= settings.tnit.maxVendors) {
                            return res.status(500).json({ error: `Only ${settings.tnit.maxVendors} vendors or admins are allowed. Please increase the number of total vendor in settings.` })
                        }
                    }
                }

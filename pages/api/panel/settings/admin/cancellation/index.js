...
export default async function handler(req, res) {
    ...
    form.parse(req, async (error, fields, files) => {
        ...
        try {
            ...
            if (req.method == 'POST') {
                var updateFields = {
                    'admin.cancellation.allowCancellation': fields.allowCancellation,
                }
                // Multi vendor logics >>>>>>>>>>>>>>
                ...
                if (settings.tnit.multiVendorAllowed) {
                    ...
                    updateFields['admin.cancellation.letOwnerManageCancellation'] = fields.letOwnerManageCancellation;
                }
                // Update setting >>>>>>>>>>>>>>
                try {
                    ...
                    return res.status(500).json({ error: `Updating setting failed. Please try again. ${process.env.NODE_ENV == 'dev' && `Error : ${error}`}` })
                }
                ...
                try {
                    ...
                    return res.status(500).json({ error: `Fetching cancellation settings failed. ${process.env.NODE_ENV == 'dev' && `Error : ${error}`}` });
                }
            } else {

        ...
        try {
            ...
            if (req.method == 'POST') {
                try {
                    ...
                    var updateFields = {
                        'policy': {
                            'refundPolicy': fields.refundPolicy,
                            'privacyPolicy': fields.privacyPolicy,
                            'TermAndConditions': fields.TermAndConditions,
                        }
                    }
                    // Update setting >>>>>>>>>>>>>>
                    await settingsModel.updateOne({}, { $set: updateFields }).exec()
                    return res.status(200).json({ success: 'Policies have been updated successfully.' })
                } catch (error) {

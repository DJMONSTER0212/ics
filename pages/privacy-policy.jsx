...
const about = ({ settings }) => {
    return (
        <div className="section-lg mt-20 xs:mt-28">
            <h1 className="text-2xl xs:text-3xl lg:text-4xl text-black-500 font-semibold text-center">Privacy Policy</h1>
            <div className='mt-14 xs:mt-20 prose prose-base w-full max-w-full prose-headings:p-0 prose-headings:m-0 prose-p:m-0 space-y-[-5px]' dangerouslySetInnerHTML={{ __html: settings.policy?.privacyPolicy }}></div>
        </div>
    )
}
...
export async function getServerSideProps(context) {
    ...
    return {
        props: {
            settings,
            seo: {
                title: `Privacy Policy | ${settings.website?.name}`,

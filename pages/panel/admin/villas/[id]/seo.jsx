...
const Villa = ({ settings }) => {
    ...
    const editSubmit = async (data) => {
        ...
        const response = await fetch(`/api/panel/villas/admin/${villa._id}/seo`, {
            method: "POST",
            body: formData
        })
    ...
    useEffect(() => {
        const fetchVilla = async (id) => {
            setLoading(true)
            const response = await fetch(`/api/panel/villas/admin/${id}/seo`);
            const responseData = await response.json();
            if (responseData.data) {
                setVilla(responseData.data)  // Set data in villa
                // Set villa details in react hook form format
                const villaDetails = { ...responseData.data.seoInfo }
    ...
    return (
        <div className="px-4 sm:px-8 bg-white dark:bg-black-600 rounded-md h-auto min-h-screen">
            ...
            {loading ?
                ...
                <>
                    {/* // Action [Page links] */}
                    <PageLinks activePage='seo' villaId={router.query.id} settings={settings} />
                    {/* // Edit form */}
                    <form onSubmit={handleSubmit(editSubmit)} encType='multipart/form-data' className='w-full mt-5 bg-background dark:bg-black-500 py-4 px-4 rounded-md select-none'>
                        ...
                        <Button type='submit' loading={submitLoading} variant='primary' label='Update SEO information' />
                    </form>
                </>
            }
        </div>

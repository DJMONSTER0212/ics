...
const Villa = ({ settings }) => {
    ...
    const onSortEnd = ({ oldIndex, newIndex }) => {
        const newValue = arrayMove(editVillaDetailsSelectValue, oldIndex, newIndex);
        setEditVillaDetailsSelectValue(newValue);
    };
    ...
    // For edit villa >>>>>>>>>>>>>>>>>
    const [editVillaDetailsSelectValue, setEditVillaDetailsSelectValue] = useState()
    ...
    const editSubmit = async (e) => {
        ...
        // Sending villa details array
        let villaDetails = JSON.stringify(editVillaDetailsSelectValue.map((detail) => detail.value))
        // Adding data to formdata
        formData.append('villaDetails', villaDetails);
        const response = await fetch(`/api/panel/villas/admin/${villa._id}/details`, {
            method: "POST",
            body: formData
        })
    ...
    useEffect(() => {
        const fetchVilla = async (id) => {
            setLoading(true)
            const response = await fetch(`/api/panel/villas/admin/${id}/details`);
            const responseData = await response.json();
            if (responseData.data) {
                setVilla(responseData.data)  // Set data in villa
                // Convert villa details in react select form
                const villaDetails = responseData.data.villaDetails.map((detail) => ({
                    value: detail,
                    label: detail
                }));
                setEditVillaDetailsSelectValue(villaDetails) // Set data in villa details
            } else {
                setGlobalMessage({ message: responseData.error, type: 'error' })
            }
            setLoading(false)
    ...
    return (
        <div className="px-4 sm:px-8 bg-white dark:bg-black-600 rounded-md h-auto min-h-screen">
            ...
            {loading ?
                ...
                <>
                    {/* // Action [Page links] */}
                    <PageLinks activePage='details' villaId={router.query.id} settings={settings} />
                    <div className='w-full mt-5 bg-background dark:bg-black-500 py-4 px-4 rounded-md select-none'>
                        ...
                        <form onSubmit={editSubmit} encType='multipart/form-data'>
                            <SortableSelect
                                ...
                                isMulti
                                defaultOptions={true}
                                loadOptions={editVillaDetailsSelectValue}
                                value={editVillaDetailsSelectValue}
                                onChange={(values) => { setEditVillaDetailsSelectValue(values) }}
                                label="Manage details"
                                placeholder='Search here...'
                                noOptionsMessage={() => 'Type to create villa details..'}
                                components={{
                                    Option: ({ data, ...props }) => {
                                        return (
                            ...
                            />
                            <p className={`mt-2 mb-3 text-base text-green-600 dark:text-green-400 font-medium`}>Tip: <span className='text-black-500 dark:text-white font-normal'>Hold and move cursor on details to reorder them. You can add details like 3 Bedroom, 1 Pool, 1 Kitchen, etc.</span></p>

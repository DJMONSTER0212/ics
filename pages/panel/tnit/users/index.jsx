...
import AddUser from '@/components/panel/pageComponents/users/tnit/AddUser'
import EditUser from '@/components/panel/pageComponents/users/tnit/EditUser'
import DeleteUser from '@/components/panel/pageComponents/users/tnit/DeleteUser'
...
const Users = ({ settings }) => {
  ...
  // API Url
  const [url, setUrl] = useState('/api/panel/users/tnit?')
  // Search options
  const searchOptions = [
    {
      name: 'All',
  ...
  const updateUserType = (userType) => {
    ...
    setUrl(`/api/panel/users/tnit?userType=${userType}&`)
